...
- **日常標準版（`main` 分支，v1.x）**：推薦主力日常使用。純淨、堅固、極致省電，持續維護並整合了底層硬體與系統相容性修正（含三星物理光感適配與防抖抗誤觸調優）。
- **紀念典藏版（`easter-egg` 分支，v2.1.5）**：永久留檔封存的特殊紀念特刊。保持最初始的程式碼與提交歷史，內建專屬終端音樂播放器彩蛋（在關於頁連擊版本號 7 次觸發）。

兩個版本使用同一個簽名，套件名稱相同，裝了一個再裝另一個需要先解除安裝。
