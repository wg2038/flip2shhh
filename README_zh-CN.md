...
- **main 分支**（v1.x）：本页描述的全部功能。
- **easter-egg 分支**（v2.1.5）：同样的底座，多一个终端播放器彩蛋，在关于页连点版本号七次。它内嵌了一首歌和歌词，用一个模仿 Linux 终端的界面播放。关于这个彩蛋的来历，代码里写得比这里清楚。

两个分支使用同一个签名，包名相同，装了一个再装另一个需要先卸载。
